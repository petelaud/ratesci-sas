...
*** Stratified OR example from Gart 1985 (tests/v_scoreci.sas DS7b);
*** Compare against R ratesci output with cc=FALSE;
DATA DS7b;
...
;
run;
* ratesci gives the corresponding CI without cc as (1.4156, 6.7747);
title "Stratified OR, expect (1.4156, 6.7747)";
%SCORECI(DS=DS7b, DELTA=., LEVEL=0.95, STRATIFY=TRUE, WEIGHT=3, ORBIAS=FALSE, BCF=FALSE, SKEW=FALSE, contrast=OR);
* and with the bias and skewness corrections: (1.3584, 6.5985);
title "Stratified OR with bias and skewness corrections, expect (1.3584, 6.5985)";
%SCORECI(DS=DS7b, DELTA=., LEVEL=0.95, STRATIFY=TRUE, WEIGHT=3, ORBIAS=TRUE, BCF=FALSE, SKEW=TRUE, contrast=OR);

*** Stratified RR on the same data (Gart & Nam 1988);
* Gart-Nam skewness-corrected, INV weighting: (1.31, 5.08);
title "Stratified RR, skewness-corrected, expect (1.31, 5.08)";
