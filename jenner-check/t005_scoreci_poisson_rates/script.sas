/* Jenner compatibility bundle — derived from petelaud/ratesci-sas.
   The %SCORECI macro below is scoreci.sas with a small set of mechanical,
   semantics-preserving syntax adaptations so it runs on the Jenner engine
   (see meta.json "notes" for the exact list). The example data and macro
   calls are taken from tests/v_scoreci.sas (published examples). */
...
* Version and date : v0.2.0 15 Jun 2025
...
*                  rates using the method of Miettinen and Nurminen (1985), 
...
*                           (TRUE/FALSE)
*                  SKEW   = indicator for inclusion of skewness correction for confidence interval and tests
*                           FALSE - Miettinen and Nurminen asymptotic score method
...
*                           1 = MH (Mantel-Haenszel: (N1i * N0i)/(N1i + N0i), default for RD and RR
...
*                : Clarified "stratum" and added repository link in header;
...
*                : Updated code to use STHETA for homogeneity tests for RR and OR
...
data dscheck;
  set &ds.;
  row = _n_;
  if (n1 = 0 or n0 = 0) | (e1 = 0 and e0 = 0 and ("&contrast." = "RR" | "&contrast." = "OR")) then do;
...
%if "&stratify." = "TRUE" and &nst. = 1 %then %do;
...
*** for unstratified case, set weights = equal and number of strata = 1;
  %let nst = 1;
  data weights;
    set dscount(rename = (N1=N_11 N0=N_01 e1=e_11 e0=e_01));
    delta = &delta.;
	  level = &level.;
    alph = 1 - level;
    W_1 = 1;
...
*** convert to wide data structure;
  ...
  data weights(keep = %do j = 1 %to &nst.; N_1&j. e_1&j. N_0&j. e_0&j. W_&j. %end; delta alph level);
...
*** set up arrays for calculations to be repeated across strata;
    array N1{&nst.} %do j = 1 %to &nst.; N_1&j. %end;;
    array N0{&nst.} %do j = 1 %to &nst.; N_0&j. %end;;
    array CMH{&nst.};
    array WTU(&nst.) %do j = 1 %to &nst.; WT_USER&j. %end;;
    array W_{&nst.};

    do i = 1 to &nst.;
      CMH[i] = (N1[i] * N0[i]) / (N1[i] + N0[i]);
      if &weight. = 1 then W_[i] = CMH[i];
...
*** with nstrat=1;
data validate(keep = %do j = 1 %to &nst.; N_1&j. N_0&j. e_1&j. e_0&j. W_&j. %end; R1S R0S SDIFF level LL UL 
  ...
  *** set up arrays for various calculations to be repeated across strata;
  array N1{&nst.} %do j = 1 %to &nst.; N_1&j. %end;;
  array N0{&nst.} %do j = 1 %to &nst.; N_0&j. %end;;
  array C1{&nst.} %do j = 1 %to &nst.; e_1&j. %end;;
  array C0{&nst.} %do j = 1 %to &nst.; e_0&j. %end;;
  array W_{&nst.};
  array WR1{&nst.};
  array WR0{&nst.};
  array MV{&nst.};
  ...
  * WEIGHTED TEST SUBROUTINE to obtain 1-sided "Bias-corrected stratified 
    ...
    NB The SAS/STAT v12.1 (SAS9.3) unstratified version of this test does not incorporate 
  	   the NT/(NT-1) correction term, because Farrington and Manning consider it 
  	   to be negligible for large samples. We (like STATXACT) include the 
       correction for consistency with the calculated Miettinen and Nurminen 
  ...
  *****************************************************************************;
  %if "&delta." ne "." %then %do;
    ...
    link mle;  *Solve the MLE equations as per Farrington and Manning, 
 ...
 *************************************END OF TEST SUBROUTINE********************;
  ...
  *bisection method;
  ...
  if (iter = 1 and count = 1) then Zzero = ZOBS; * save test statistic at D=0 for CMH test;
  ...
  * deal with special cases at +/- 1 not requiring iteration: 
  * e.g. if point estimate is -1 then so is LCL; 
  if (((Zminus1 <= Z) and "&contrast." ne "OR")  |
      ("&contrast." = "OR" and abs(D2+1) < 1E-8)) then do;
    if (iter = 1) then do; D2 = -1; incr = 0; end;
    if (iter = 2) then do; D2 = -1; incr = 0; end;
    if (iter = 4) then do; D2 = -1; incr = 0; end;
  end;
  if (((Zplus1 >= Z) and "&contrast." ne "OR") | 
      ("&contrast." = "OR" and abs(D1-1) < 1E-8)) then do;
  ...
  *** or [0, +inf] for binomial or Poisson RR;
  ...
  IF (incrt > &converge. and COUNT < &maxiter. ) THEN do;
    GOTO LOOPT;
  end;

  IF (ITER = 1 and (ERR = 0 | incrt < &converge. | count = &maxiter.)) THEN do;
    pt_est = D1t;
    pt_back = D1;
  end;
  IF (ITER = 2 and (ERR = 0 | incrt < &converge. | count = &maxiter.)) THEN LL = D1t;
  IF (ITER = 3 and (ERR = 0 | incrt < &converge. | count = &maxiter.)) THEN UL = D1t;
  IF (ITER = 4 and (ERR = 0 | incrt < &converge. | count = &maxiter.)) THEN uc_est = D1t;
 ...
 ********************************HOMOGENEITY TEST SUBROUTINE ****************;
  ...
  ********************************END OF HOMOGTEST SUBROUTINE****************;
  ...
  mle:
  * solve the MLE equations as per Farrington and Manning, 
  * including stratified element as per Miettinen and Nurminen;
  do i = 1 to &nst.;
    NT = N1[i] + N0[i];
    CT = C1[i] + C0[i];
    R1 = C1[i]/N1[i]; 
	...
	%if "&contrast." = "RD" %then %do;
     %if "&distrib." = "bin" %then %do;
      ...
      if (((R1S = 0 and R0S = 0) | (R1S = 1 and R0S = 1)) and D = 0) then MU3[i] = 0; 
    ...
    * Transform D value between -1 and +1 to (-inf, +inf) for Poisson RD;
	...
	%if "&contrast." = "RR" %then %do;
      * Transform D (theta) value between -1 and +1 to (0, +inf) for RR;
      if D = -1 then do;
        if iter = 2 and skipt = 0 then Dr = 0;
        else Dr = 1E-12;
	  end;
      else if D = 1 then do;
        if iter = 3 and skipt = 0 then Dr = .I;
        else Dr = 1E12;
	  end;
      ...
      %if "&distrib." = "bin" %then %do;
        ...
        * Fix for special case resulting in problems for stratified score;
        MR0 = max(MR0, 1E-10);
        %if "&stratify." = "TRUE" and (&weight. = 2 or &weight. = 3) %then %do;
		...
		%end;
        %if "&stratify." ne "TRUE" or (&weight. ne 2 and &weight. ne 3) %then %do;
          STHETA[i] = R1 - R0 * Dr;
          MV[i] = max(0, (MR1*(1 - MR1)/N1[i] + 
	...
	%if "&contrast." = "OR" %then %do;
      * Transform D (theta) value between -1 and +1 to (0, +inf) for OR;
      if D = -1 then do;
        ...
        if iter = 3 and skipt = 0 then Dr = 1E12;
        else Dr = 1E12;
	  end;
      else Dr = round(tan(pi * (min(D, (1 - 1E-20)) + 1) / 4), &converge./100);
	  Dt = Dr;
      ...
      * If A=0 then we solve a linear equation instead;
      ...
      %if "&orbias." = "TRUE" and "&contrast." = "OR" %then %do;
...
DATA RESULT(drop = r1s r0s sdiff pt_est ll ul zzero
                   homogq homogp 
					%do j = 1 %to &nst.; N_1&j. N_0&j. e_1&j. e_0&j. w_&j. %end; delta level incr count
				  %if "&stratify."="FALSE" %then %do; 
				    w_1 
				  %end;
				  );
...
%if "&stratify." = "TRUE" %then %do;
DATA WEIGHTING(keep = %do j = 1 %to &nst.; W_&j. %end; n_strata weight);
...
*** Cisapride meta-analysis dataset (tests/v_scoreci.sas DS2),
*** analysed as Poisson exposure-adjusted rates (DISTRIB=poi),
*** alongside the binomial analysis for comparison;
DATA DS2;
...
;
run;
*** Binomial analysis first - match published results;
*** Method=SCAS with weights=MH: (0.245523, 0.370330);
title "Cisapride binomial SCAS expect (0.245523, 0.370330)";
%SCORECI(DS=DS2, LEVEL=0.95, STRATIFY=TRUE, SKEW=TRUE, WEIGHT=1);
*** Method=MN (R ratesci::scoreci): (0.245286, 0.369981);
title "Cisapride binomial MN expect (0.245286, 0.369981)";
%SCORECI(DS=DS2, LEVEL=0.95, STRATIFY=TRUE, SKEW=FALSE, WEIGHT=1);
*** Poisson exposure-adjusted analysis of the same table;
title "Stratified Poisson RD, skewness-corrected";
%SCORECI(DS=DS2, LEVEL=0.95, STRATIFY=TRUE, SKEW=TRUE, WEIGHT=1, DISTRIB=poi);
title "Stratified Poisson RD, Miettinen-Nurminen";
