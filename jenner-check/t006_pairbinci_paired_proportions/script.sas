/* Jenner compatibility bundle — derived from petelaud/ratesci-sas.
   The %PAIRBINCI macro below is pairbinci.sas with a small set of mechanical,
   semantics-preserving syntax adaptations so it runs on the Jenner engine
   (see meta.json "notes" for the exact list). The example calls are taken
   from tests/v_pairbinci.sas (published examples, Fagerland et al. 2014
   and Laud 2025). */
...
%macro fx(x); 
  ...
  if abs(mu3) < 1E-10 or abs(V) < 1E-10 then scterm = 0;
...
%macro Tango (a, b, c, d, X0, X1, Z);
  ...
  do until (abs(change) < 10**-(&dps.+1));* or fx1 = 0);* or fx0 <= 0);
    %if "&contrast." = "RR" %then %do;
	  if x0 = -1 then x0t = tan(pi * (1E-12)/4); 
	  else if x0 = 1 then x0t = 1E12;
...
*** Tango method for contrast=RD: (-0.517, -0.026) in Fagerland Table V;
title "Tango RD expect (-0.517, -0.026)";
%pairbinci(1,1,7,12,level=0.95, bcf=FALSE, skew=FALSE, contrast = RD, dps = 3);

*** Tang method for contrast=RR: (0.065, 0.907) in Fagerland Table VI;
title "Tang RR expect (0.065, 0.907)";
%pairbinci(1,1,7,12,level=0.95, bcf=FALSE, skew=FALSE, contrast = RR, dps = 3);

*** SCAS method for contrast=RD (Laud 2025 Table 5.1): (-0.528, -0.018);
title "SCAS RD expect (-0.528, -0.018)";
%pairbinci(1,1,7,12,level=0.95, bcf=TRUE, skew=TRUE, contrast = RD, dps = 3);
*** SCAS method for contrast=RR (Laud 2025 Table 5.2): (0.043, 0.928);
title "SCAS RR expect (0.043, 0.928)";
%pairbinci(1,1,7,12,level=0.95, bcf=TRUE, skew=TRUE, contrast = RR, dps = 3);
title;

*** Same data via PROC FREQ AGREE: McNemar chi-square 4.5 (p=0.0339)
*** agrees with the Tango test for association above;
data Students;
