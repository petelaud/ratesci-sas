**********************************************************
*
* Program Name   : V_SCORECI3rr.SAS
* Level / Study  : global reusable macro
* Type           : macro validation
* Description    : Validation of SCORECI macro for contrast = RR with STRATIFY=TRUE
*                  Use in combination with R code (v_scoreci3rr.R)
*					(Work in progress)  
* 
* Author         : Pete Laud 
* 
...
*
*                  Discrepancies found:
*                  sasLM::RRmn cannot cope with zero event counts
*                  sasLM::RRmn estimates p2/p1 if p2 > p1
*                  sasLM::RRmn estimates of RR are inconsistent with p1 and p2
*
**********************************************************;
...
%macro runstrat(skew=FALSE, distrib=bin, contrast=RR);
...
proc export data=sasval 
 outfile = "&path.tests\sasval3rr.csv"
 dbms = csv replace;
run;
...
proc export data=sasval 
 outfile = "&path.tests\sasval3rrskew.csv"
 dbms = csv replace;
run;
...
*Export to csv for validation against R output using program v_scoreci.R; 
proc export data=sasval 
 outfile = "&path.tests\sasval3prr.csv"
 dbms = csv replace;
run;
...
*Export to csv for validation against R output using program v_scoreci.R; 
proc export data=sasval 
 outfile = "&path.tests\sasval3prrskew.csv"
 dbms = csv replace;
run;
